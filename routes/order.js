...
router.get("/", async (req, res) => {
  try {
    ...
    if (!orders)
      return res
        .status(404)
        .send({ success: false, message: "no orders to be returned" });
    return res.status(200).send(orders);
  } catch (err) {
    return res.status(300).send({ success: false, message: err.message });
  }
});
...
router.get("/get/userOrders/:userid", async (req, res) => {
  try {
    ...
    if (!userOrder)
      return res
        .status(404)
        .send({ success: false, message: "user has not made any orders" });
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!order)
      return res.status(404).send({ success: false, message: "invalid order" });
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (!order)
      return res.status(400).send({ success: false, message: "invalid order" });
    return res.status(200).send(order);
  } catch (err) {
    return res.status(500).send({ success: false, message: err.message });
  }
...
router.delete("/:id", async (req, res) => {
  try {
      ...
      async (order) => {
        if (order) {
          ...
          return res.status(200).send({
            success: true,
            message: "the order item has been deleted",
          });
        } else {
          return res
            .status(400)
            .send({ success: true, message: "the order cannot be deleted" });
        }
      }
    );
    ...
    return res
      .status(200)
      .send({ success: true, message: "order has been deleted" });
  } catch (err) {
    return res.status(500).send({ success: false, message: err.message });
  }
});
...
router.post("/", async (req, res) => {
  console.log(req.body.user.useId);
  try {
...
router.get("/get/count", async (req, res) => {
  try {
    ...
    if (!orderCount)
      return res.status(500).json({ success: false, message: "no order" });
